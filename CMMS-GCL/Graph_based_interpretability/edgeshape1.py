...
def edgeshaper(model, data, x, E, batch, smi_em, M=100, target_class=1, P=None, log_odds=True, seed=42): 
    ...
    for j in range(num_edges):
        ...
        for i in range(M):
            ...
            V_j_plus = out_prob[:, 0].asnumpy().item() if target_class == 0 else 1 - out_prob[:, 0].asnumpy().item()
            ...
            V_j_minus = out_prob[:, 0].asnumpy().item() if target_class == 0 else 1 - out_prob[:, 0].asnumpy().item()
